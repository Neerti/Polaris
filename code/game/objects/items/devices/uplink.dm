	...
	if(nanoui_menu == 0)
		...
		for(var/datum/data/record/L in data_core.locked)
			if(L.fields["id"] == exploit_id)
				...
				nanoui_data["exploit"]["residence"] =  html_encode(L.fields["residence"])
				nanoui_data["exploit"]["party_affiliation"] =  html_encode(L.fields["party_affiliation"])
				nanoui_data["exploit"]["employer"] =  html_encode(L.fields["employer"])
