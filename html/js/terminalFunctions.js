...
//Adds the provided string to the last row (20) after shuffling all the other lines up one row (deleting the top line)
function addRowConsole(string) {
	for (var i = 1; i < 19; i++) {
		var newRow = i + 1;
		document.getElementById("row" + i).innerHTML = document.getElementById("row" + newRow).innerHTML;
	}
	document.getElementById("row19").innerHTML = string;
}
